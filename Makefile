...
deploy-K8s2: ## Deploy Kubernetes
	docker run --rm -it -v $(CURDIR):/src:rw -w /src -e VCLOUD_USERNAME=${VCLOUD_USERNAME} -e SSH_USERNAME=${SSH_USERNAME} -e SSH_PASSWORD=${SSH_PASSWORD} -e VCLOUD_PASSWORD=${VCLOUD_PASSWORD} -e VCLOUD_HOST=${VCLOUD_HOST} -e VCLOUD_ORG=${VCLOUD_ORG} -e VCLOUD_VDC_NAME="${VCLOUD_VDC_NAME}" $(PYVCLOUD_IMAGE) python ansible/deploy-ansible.py  vmware/sdk/tenant.yaml
